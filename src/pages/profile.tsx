...
export default function ProfilePage() {  
  let auth = useAuth();
  const [form, setValue] = useState<TName>({ email: '', password: '' ,name:''});
  ...
  useEffect(() => {
    if (auth!.user) setValue({email: auth!.user, password: '' ,name:auth!.user})
  }, [setValue,auth!.user]);
...
изменить свои персональные данные</p>
       ...
       <span className={`${styles.field}`}>
        ...
        <span className={`${styles.field} pt-6`}>
        <PasswordInput  name="password" value="" onChange={onChange}></PasswordInput>
       </span>
       </div>
       )}
        {(isOrders || isOrder) && (
